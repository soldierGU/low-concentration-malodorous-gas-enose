# -*- coding: utf-8 -*-
"""Train SmellNet/ScentFormer on the cached 5-fold split.
...
from model_for_compare.ref_model17_SmellNet import Ref17_SmellNetScentFormer
...
BATCH = 32
EPOCHS = 200
OUT_DIR = "compare_models_5fold_SmellNet"
...
def build_model(args):
    return Ref17_SmellNetScentFormer(
        num_channels=NUM_CH,
        ...
        pool="cls" if args.use_cls_token else "mean",
        use_temporal_diff=args.use_temporal_diff,
    )
...
def train_one_fold(fold_idx, args):
    ...
    save_dir = os.path.join(args.out_dir, "SmellNet", f"fold{fold_idx}")
    ...
    best_f1 = -1.0
    ckpt_path = os.path.join(save_dir, f"best_SmellNet_fold{fold_idx}.pt")
    latest_path = os.path.join(save_dir, f"latest_SmellNet_fold{fold_idx}.pt")
    history_path = os.path.join(save_dir, f"history_SmellNet_fold{fold_idx}.csv")
    start_epoch = 1
    ...
    if args.resume:
        if os.path.isfile(latest_path):
            ...
            best_f1 = float(latest.get("best_f1", -1.0))
            start_epoch = int(latest["epoch"]) + 1
            print(f"[SmellNet F{fold_idx}] resume from latest epoch {start_epoch - 1}", flush=True)
        elif os.path.isfile(ckpt_path) and os.path.isfile(history_path):
            model.load_state_dict(torch.load(ckpt_path, map_location="cpu"))
            with open(history_path, "r", newline="", encoding="utf-8") as f:
                rows = list(csv.DictReader(f))
            if rows:
                start_epoch = int(rows[-1]["epoch"]) + 1
                best_f1 = max(float(row["val_f1"]) for row in rows)
                for _ in range(start_epoch - 1):
                    scheduler.step()
                print(
                    f"[SmellNet F{fold_idx}] resume from best checkpoint; "
                    f"history last epoch {start_epoch - 1}, best_f1={best_f1:.2f}",
                    flush=True,
                )
    ...
    if start_epoch > args.epochs:
        print(f"[SmellNet F{fold_idx}] already reached epoch {start_epoch - 1}; skip training.", flush=True)
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        if epoch == 1 or epoch % 10 == 0:
            print(
                f"[SmellNet F{fold_idx} Ep{epoch:03d}] "
    ...
    summary = {
        ...
        "use_temporal_diff": bool(args.use_temporal_diff),
        "val_loss": val_loss,
        "val_acc1": val_acc1,
        "val_acc5": val_acc5,
        "val_f1": val_f1,
    ...
    with open(os.path.join(save_dir, f"val_summary_SmellNet_fold{fold_idx}.json"), "w", encoding="utf-8") as f:
...
def model_from_summary(summary):
    return Ref17_SmellNetScentFormer(
        ...
        num_classes=NUM_CLASSES,
        seq_length=NUM_ROWS,
        model_dim=int(summary["model_dim"]),
        num_heads=int(summary["num_heads"]),
        num_layers=int(summary["num_layers"]),
        dropout=float(summary["dropout"]),
        use_cls_token=bool(summary.get("use_cls_token", False)),
        pool="cls" if bool(summary.get("use_cls_token", False)) else "mean",
        use_temporal_diff=bool(summary.get("use_temporal_diff", False)),
    )
...
def evaluate_test_for_folds(out_dir, batch_size):
    ...
    for fold_idx in range(1, 6):
        val_summary_path = os.path.join(out_dir, "SmellNet", f"fold{fold_idx}", f"val_summary_SmellNet_fold{fold_idx}.json")
        ...
        print(f"[SmellNet Test F{fold_idx}] Acc={test_acc1:.2f}, F1={test_f1:.2f}, Recall={test_recall:.2f}", flush=True)
    return fold_rows
...
def write_outputs(out_dir, val_rows, test_rows, resource_row):
    val_csv = os.path.join(out_dir, "smellnet_val_per_fold.csv")
    with open(val_csv, "w", newline="", encoding="utf-8") as f:
        fieldnames = [
            "model", "fold", "model_dim", "num_heads", "num_layers", "dropout",
            "use_cls_token", "use_temporal_diff",
    ...
    test_csv = os.path.join(out_dir, "smellnet_test_per_fold.csv")
    ...
    with open(os.path.join(out_dir, "smellnet_final_summary.json"), "w", encoding="utf-8") as f:
        json.dump(final, f, indent=2)
    final_csv = os.path.join(out_dir, "smellnet_final_summary.csv")
...
def parse_args():
    ...
    parser.add_argument("--resume", action="store_true", help="Resume interrupted fold training from latest/best checkpoint and existing history.")
    ...
    parser.add_argument("--use-temporal-diff", action="store_true")
...
def main():
    ...
    if not args.skip_train:
        ...
        for fold_idx in range(1, 6):
            path = os.path.join(args.out_dir, "SmellNet", f"fold{fold_idx}", f"val_summary_SmellNet_fold{fold_idx}.json")
